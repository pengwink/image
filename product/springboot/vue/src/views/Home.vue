  ...
  <div id="main">
    ...
    <div id="image-content">
      <div class="image-item" v-for="(item, index) in images" :key="index" :style="reactiveImage">
        ...
        <el-image :src="item.img" :preview-src-list="getSrcList(index)" class="image">
  ...
  methods: {
    ...
    downloadImage() {
      ...
      for (let i = 0; i < imgIndex.length; i++) {
        ...
        let imgName = this.images[imgIndex[i]].name;
    ...
    selectAllImage(mode, value) {
      ...
      if (mode == "type") {
        ...
        requestApi = '/photo/selectAllByType';
      }
      if (mode == "time") {
        formData.append("imageDate", value);
        requestApi = '/photo/selectAllByTime';
      }
      this.axios({
        url: this.$serveUrL + requestApi,
        headers: {
        ...
        if (resp.data.status == "success") {
          ...
          for (var i = 0; i < _this.previewImageUrL.length; i++) {
            _this.previewImageUrL[i] = _this.previewImageUrL[i]
          }
    ...
    handleCheckIndexToId() {
      ...
      for (let i = 0; i < len; i++) {
        this.checkedImgId[i] = this.images[this.checkedImgIndex[i]].id;
      }
    },
    // 部分被选中,当勾选时，上方显示操作菜单
