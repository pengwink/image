...
<!--        <img :src="img.img" @click="showdia(img)">-->
<!--        @click="showdia(img)"-->
  ...
  methods: {
    sortacitve(key){
      ...
      for (let i=0; i<this.sorts.length; i++) {
        ...
        if (this.sorts[i].id==key) {
          this.sorts[i].flag=true
        }
      }
      this.getdata(key)
    },
    getdata(tid){
      this.request.get("/type/photos/" + tid, {
      }).then(res => {
        this.imgs=res.data
    ...
    gettype(){
      this.request.get("/type/photos", {
      }).then(res => {
        this.sorts = res.data
        this.sorts[0].flag=true;
        this.getdata(this.sorts[0].id)
